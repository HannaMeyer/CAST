...
The objective of this vignette is to showcase the CV methods implemented in `CAST`. To do so, we will work with two datasets of annual average air temperature and fine Particulate Matter (PM$_{2.5}$) air pollution in continental Spain for 2019, for which several predictors have been collected. For more details, check [Milà et al. (2024)](https://doi.org/10.5194/gmd-17-6007-2024) where the dataset is described in detail. 



```{r, message=FALSE, warning=FALSE, echo=FALSE}
try(temperature <- read_sf("https://github.com/carlesmila/RF-spatial-proxies/raw/main/data/temp/temp_train.gpkg"), silent=TRUE)
try(pm25 <- read_sf("https://github.com/carlesmila/RF-spatial-proxies/raw/main/data/AP/PM25_train.gpkg"), silent=TRUE)
try(spain <- read_sf("https://github.com/carlesmila/RF-spatial-proxies/raw/main/data/boundaries/spain.gpkg"), silent=TRUE)
data_exist <- exists("temperature") && exists("pm25")  && exists("spain") && !is.null(temperature) && !is.null(pm25)&& !is.null(spain)
```


```{r read data, fig.width=5, fig.height=7, eval = data_exist}
# Read data
...
```{r geodist density, fig.width=6, fig.height=7, eval = data_exist}
# Random 5-fold CV
...
```{r geodist ecdf, fig.width=6, fig.height=7, eval = data_exist}
# Plot ECDF functions
...
```{r NNDM LOO temp, fig.width=6, fig.height=4, eval = data_exist}
...
```{r NNDM LOO pm25-1, fig.width=6, fig.height=4, eval = data_exist}
...
```{r NNDM LOO pm25-2, fig.height=4, fig.width=6, echo=FALSE, eval = data_exist}
...
```{r model fitting temp NNDM, eval = data_exist}
# LOO CV
...
```{r model fitting pm25 NNDM, eval = data_exist}
# LOO CV
...
```{r parse results LOO CV, echo=FALSE, eval = data_exist}
rbind(
...
If you want to learn all the finer details of kNNDM and check our simulation study evaluating its performance, [here](https://doi.org/10.5194/gmd-17-5897-2024) is the link to the document (also listed at the end of the vignette). Now let's apply kNNDM to our data! First, for temperature with the default clustering algorithm:

```{r kNNDM temp, fig.width=6, fig.height=4, eval = data_exist}
...
```{r kNNDM pm25, fig.width=6, fig.height=4, eval = data_exist}
...
```{r kNNDM pm25 v2, fig.width=6, fig.height=4, eval = data_exist}
...
```{r kNNDM viz, fig.width=7, fig.height=3, echo=FALSE, eval = data_exist}
p1 <- ggplot() +
  geom_sf(data = spain, fill = "grey", alpha = 0.1) +
...
```{r model fitting temp kNNDM, eval = data_exist}
# Random 5-fold CV
...
```{r model fitting pm25 kNNDM, eval = data_exist}
# Random 5-fold CV
...
```{r parse results kfold CV, echo=FALSE, eval = data_exist}
rbind(
