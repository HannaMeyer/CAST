...
```{r, message = FALSE, warning=FALSE}
plot(geodist(splotdata,predictors_sp,CVtest=model_default$control$indexOut))+ 
  scale_x_log10(labels=round)
plot(geodist(splotdata,predictors_sp,CVtest=indices_LLO$indexOut))+ 
  scale_x_log10(labels=round)
plot(geodist(splotdata,predictors_sp,CVtest=indices_knndm$indx_test))+ 
  scale_x_log10(labels=round)
```
...
* Linnenbrink, J., Milà, C., Ludwig, M., and Meyer, H.: kNNDM (2024):k-fold Nearest Neighbour Distance Matching Cross-Validation for map accuracy estimation. Geosci Model Dev., 17, 5897–5912.
